...
const Reports = () => {
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    // معالجة دقيقة لتاريخ البداية (من الساعة 00:00:00)
    ...
    // معالجة دقيقة لتاريخ النهاية (حتى الساعة 23:59:59)
  ...
  // ✅ استخدام جميع المهام لجلب أسماء الشركات لكي لا تختفي عند الاختيار
  const allUniqueCompanies = [...new Set(tasks.map((t) => t.company).filter(Boolean))];
  
  // لغرض الرسوم البيانية نستخدم المفلترة فقط
  ...
  function exportPDF() {
    ...
    const userName = "System User"; // لاحقًا نربطه بالـ Auth
    ...
    doc.setFontSize(10);
    doc.text(`Total Tasks: ${summary.totalTasks}`, 14, 48);
    doc.text(
      `Total Time: ${formatMinutesToHM(summary.totalMinutes)}`,
      14,
      54
    );
    doc.text(
      `Most Common Task: ${summary.mostCommonTask}`,
      14,
      60
    );
  ...
  function exportExcel() {
    const userName = "System User"; // لاحقًا من Auth
    const today = new Date().toLocaleString();
    ...
    const summarySheetData = [
      ...
      { Field: "Total Tasks", Value: summary.totalTasks },
      {
        Field: "Total Time",
        Value: formatMinutesToHM(summary.totalMinutes),
      },
      {
        Field: "Most Common Task",
        Value: summary.mostCommonTask,
      },
    ];
    ...
    const tasksData = filteredTasks.map((t) => ({
      ...
      Type: t.type,
      Worker: t.workerName,
      Time: formatMinutesToHM(t.timeSpent),
    }));
